...
def problem2_trip():
    ...
    yes_command = ["YES", "yes", "Y", "Yes", "y", "ja", "JA", "J", "j"]
    intensity_cals = {"easy": 300, "medium": 450, "hard": 600}
    total_hours = float(input("How many hours did you walk up Ulriken? "))
    if total_hours > 0:
        Yes_no = input("Where they all the same insensity?")
        while True:
            if Yes_no in no_command:
                while True:
                    easy_hours = float(input("How many of the hours where a easy intensity?"))
                    normal_hours = float(input("How many of the hours where normal intensity?"))
                    hard_hours = float(input("How many of the hours where a hard intensity?"))
                    if easy_hours + normal_hours + hard_hours < 0:
                        print("cant walk negative hours!")
                        continue
                    elif round(easy_hours + normal_hours + hard_hours, 2) == 0:
                        print("You did not walk Ulriken!")
                        continue
                    elif round(easy_hours + normal_hours + hard_hours, 2) != round(total_hours, 2):
                        print(f"You said you walked {total_hours} hours!")
                        continue
                    else:
                        total_cals_burned = easy_hours * intensity_cals["easy"] + normal_hours * intensity_cals[
                            "medium"] + hard_hours * intensity_cals["hard"]
